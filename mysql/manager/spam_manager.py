...
class SpamManager:
    ...
    def add_spam_phone(target_phone):
        ...
        try:
            ...
            check_sql = "SELECT * FROM spam_number WHERE phone = ? LIMIT 1"
    ...
    # spam 테이블에서 보이스피싱의심번호인지 조회, 맞다면 count 1 증가하고 count 반환
    @staticmethod
    def check_spam_phone(target_phone):
        ...
        try:
            ...
            check_sql = "SELECT count FROM spam_number WHERE phone = ? LIMIT 1"
            cursor.execute(check_sql, (target_phone,))
            result = cursor.fetchone()

            if result:
                update_sql = "UPDATE spam_number SET count = count + 1 WHERE phone = ?"
...
def conn_sqlite():
    return sqlite3.connect('KB_AI.db')
